...
module.exports.delete = async (req, res) => {
    ...
    if(blog.user !== userId) {
        return res.status(403).json({ message: "Unauthorized Forbidden, you cancot delete someone else blog", status: "error" });
    }
...
module.exports.addVote = async (req, res) => {
    ...
    if(blog.votedBy.includes(userId)){
        return res.status(400).json({ message: "user has already voted", status: "error" })
    }
